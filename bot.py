...
async def cmd_start(message: types.Message):
    ...
    text = (
        "👋 <b>Привет! Добро пожаловать в закрытую экосистему по заработку на крипте и трафике.</b>\n\n"
        "Здесь ты получаешь:\n"
        "• 📈 Крипто-сигналы с понятными точками входа/выхода\n"
        "• 🎯 Структурное обучение по трейдингу (8 модулей)\n"
        "• 🚀 Обучение по переливу трафика и работе с офферами (6 модулей)\n"
        "• 🤝 Партнёрскую программу <b>50% + 10%</b> с пожизненным доступом к материалам\n\n"
        "Мы сами вложили <b>$15 000</b> в обучение у топовых трейдеров и арбитражников, "
        "собрали выжимку и упаковали всё это в один доступ за <b>$100</b>.\n\n"
...
async def msg_edu(message: types.Message):
    ...
    text = (
        ...
        "В полном доступе у тебя будет сразу три направления:\n\n"
        "1️⃣ <b>Крипто-трейдинг</b> — 8 модулей от азов до системного подхода: психология, "
        "риск-менеджмент, точки входа, управление сделками.\n\n"
        "2️⃣ <b>Перелив трафика</b> — 6 модулей по источникам трафика, связкам, креативам и аналитике. "
        "Как собирать поток людей в бота и монетизировать его.\n\n"
        ...
        "Доступ ко всем этим направлениям и партнёрке открывается после единовременной оплаты <b>100$</b>.\n\n"
        "Выбери, с чего начать 👇"
    )

    # здесь ставь ТОТ ЖЕ reply_markup, который раньше был в send_edu_main
    # если там была функция типа training_menu_keyboard(), то:
    await message.answer(text, reply_markup=training_menu_keyboard())
...
async def cb_home_how(call: CallbackQuery):
    text = (
        ...
        "📦 <b>Что ты получаешь за $100:</b>\n"
        "• Полный доступ к обучению по трейдингу (8 модулей)\n"
        "• Полный доступ к обучению по трафику (6 модулей)\n"
        "• Доступ к закрытому каналу с сигналами на 1 месяц\n"
        "• Доступ к партнёрской программе 50% + 10%\n"
        "• Личный кабинет со статистикой и реферальной ссылкой\n\n"
        "Доступ к обучению и партнёрке — <b>навсегда</b>.\n"
        "Доступ к сигналам — по подписке (продление по $50 в месяц).\n\n"
        "🤝 <b>Партнёрка:</b>\n"
        "• 50% с покупки полного доступа людьми 1-го уровня\n"
        "• 10% с покупки полного доступа людьми 2-го уровня\n\n"
        "⚠️ <b>Важно:</b>\n"
        "Крипта и трейдинг всегда связаны с риском. Никаких гарантий дохода нет.\n"
        "Решения о входе в сделки ты принимаешь сам.\n\n"
        "❌ <b>Возврат средств не предусмотрен.</b>\n"
        "После оплаты ты сразу получаешь доступ ко всем закрытым материалам, "
        "поэтому возврат технически и юридически не делается."
    )
    try:
